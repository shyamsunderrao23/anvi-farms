...
export default function ProductsPage() {
  ...
  const categories = [
    ...
    { id: 'ghee', name: 'Pure Vedic Ghee' },
    { id: 'honey', name: 'Raw Wild Honey' },
    { id: 'oils', name: 'Wood Pressed Oils' },
    { id: 'spices', name: 'Spices & Malts' },
  ];
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    if (sortBy === 'low-high') return a.variants[0].price - b.variants[0].price;
    if (sortBy === 'high-low') return b.variants[0].price - a.variants[0].price;
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F4] font-sans text-[#1C1917] flex flex-col selection:bg-[#C28E2E] selection:text-[#0D2B1D]">
      ...
      <main className="flex-1 w-full max-w-[1720px] mx-auto px-4 sm:px-8 lg:px-12 py-10 sm:py-16 space-y-12 sm:space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-[62px] font-cursive font-normal text-[#320047] tracking-normal leading-tight py-1">
            Farm Fresh Organic Essentials
          </h1>

          <p className="text-sm text-slate-700 font-medium leading-relaxed max-w-xl mx-auto">
            100% natural, ethically harvested, and lab-tested produce delivered fresh from our estate to your doorstep.
          </p>
        </div>

        {/* PRODUCT GRID - EXACT MATCH TO HOME PAGE FARMFAVORITES */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-8 max-w-[1500px] mx-auto">
          {sortedProducts.map((product) => {
            ...
            const badgeText = product.badge || 'NEW';
              ...
              <div key={product.id} className="group flex flex-col justify-between h-full space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-1.5 pt-1">
                    <Link
                      href={`/product/${product.id}`}
                      className="font-serif font-bold text-lg text-[#1C1917] cursor-pointer hover:text-[#310048] transition-colors min-h-[56px] flex items-center justify-center leading-snug px-1 block text-center"
                    >
                      {product.name}
                    </Link>
                    <div className="font-extrabold text-[#C28E2E] text-base text-center">
